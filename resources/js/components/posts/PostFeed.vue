<template>
    <div class="py-3 my-3 z-unset border-b-2">
        <div class="flex justify-between pb-2 px-3">
            <div class="flex">
                <div class="pr-2">
                    <img class="w-10 h-10 rounded-circle object-cover" :src="`${ data.user.avatar != 'avatar.png' ? `storage/users-avatar/${data.user.avatar}` : 'images/avatar.png' }`"/>
                </div>
                <div>
                    <div><span class="font-bold capitalize">{{ data.user.name }}</span><span class="pl-2">posted an update</span></div>
            ...
            <div class="flex my-2 w-100" v-for="(comment, i) in data.comments" :key="comment.id">
                <div class="pr-2">
                    <v-avatar size="small">
                        <v-img
                            cover
                            :src="`${ comment.user.avatar != 'avatar.png' ? `storage/users-avatar/${comment.user.avatar}` : 'images/avatar.png' }`"
                ...
                <v-sheet
                    ...
                    >
                    <div class="pb-3">
                        <div class="font-bold capitalize">{{ comment.user.name }} <span class="text-xs pl-1 font-light">{{ formatDate(comment.created_at) }}</span></div>
